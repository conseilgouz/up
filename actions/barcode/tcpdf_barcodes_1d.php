...
class TCPDFBarcode {
	 ...
	 * @param @text (string) barcode text
 	 * @public
	 */
	...
	public function getBarcodeSVGcode($w=2, $h=30, $color='black') {
		...
		$svg .= "\t".'</g>'."\n";
		$svg .= '</svg>'."\n";
		return $svg; // Pascal : return instead of echo
	}
	 ...
	 * @param $align (string) barcode alignment
  	 * @return string HTML code.
 	 * @public
	 */
	public function getBarcodeHTML($w=2, $h=30, $color='black',$align='center') {
		$margin = "";
		if ($align =="center") $margin = "margin:0 auto;";
		if ($align =="left") $margin = "margin-left:0;margin-right:auto;";
		if ($align =="right") $margin = "margin-left:auto;margin-right:0;";
		$html = '<div style="font-size:0;position:relative;width:'.($this->barcode_array['maxw'] * $w).'px;height:'.($h).'px;'.$margin.'" >'."\n";
		// print bars
		$x = 0;
		foreach ($this->barcode_array['bcode'] as $k => $v) {
			$bw = round(($v['w'] * $w), 3);
...
//		header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
		...
		return "<img src='data:image/png;base64,".base64_encode ($data)."'>"; // Pascal : return instead of echo + return <img>
	}
	...
	protected function encode_code39_ext($code) {
		...
		$clen = strlen($code);
		for ($i = 0 ; $i < $clen; ++$i) {
			if (ord($code[$i]) > 127) { // php 7.4
				return false;
			}
			$code_ext .= $encode[$code[$i]];// php 7.4
		}
		return $code_ext;
	}
	...
	protected function checksum_code39($code) {
		...
		$clen = strlen($code);
		for ($i = 0 ; $i < $clen; ++$i) {
			$k = array_keys($chars, $code[$i]);// php 7.4
			$sum += $k[0];
		}
		$j = ($sum % 43);
		return $chars[$j];
	...
	protected function binseq_to_array($seq, $bararray) {
		...
		for ($i = 0; $i < $len; ++$i) {
			$w += 1;
		if (($i == ($len - 1)) OR (($i < ($len - 1)) AND ($seq[$i] != $seq[($i+1)]))) {// php 7.4
				if ($seq[$i] == '1') {// php 7.4
					$t = true; // bar
				} else {
					$t = false; // space
	...
	protected function barcode_i25($code, $checksum=false) {
		...
		for ($i = 0; $i < $clen; $i = ($i + 2)) {
			$char_bar = $code[$i];// php 7.4
			$char_space = $code[$i+1];// php 7.4
	...
	protected function barcode_c128($code, $type='') {
		...
		switch(strtoupper($type)) {
			...
			case 'C': { // MODE C
				...
				for ($i = 0; $i < $len; $i+=2) {
					$chrnum = $code[$i].$code[$i+1];// php 7.4
				...
				foreach ($sequence as $key => $seq) {
					switch($seq[0]) {
						...
						case 'C': {
							...
							for ($i = 0; $i < $seq[2]; $i+=2) {
								$chrnum = $seq[1][$i].$seq[1][$i+1];// php 7.4
								$code_data[] = intval($chrnum);
							}
							break;
						}
	...
	protected function barcode_eanupc($code, $len=13) {
		...
		if ($code_len == $data_len) {
			// add check digit
			$code .= $r;
		} elseif ($r !== intval($code[$data_len])) {// php 7.4
			// wrong checkdigit
			return false;
		}
		if ($len == 12) {
		...
		if ($upce) {
			...
			for ($i = 0; $i < 6; ++$i) {
				$seq .= $codes[$p[$i]][$upce_code[$i]];// php 7.4
			}
			...
			if ($len == 8) {
				for ($i = 0; $i < $half_len; ++$i) {
					$seq .= $codes['A'][$code[$i]];// php 7.4
				}
			} else {
				$p = $parities[$code[0]];
				for ($i = 1; $i < $half_len; ++$i) {
					$seq .= $codes[$p[$i-1]][$code[$i]];// php 7.4
				}
			}
			$seq .= '01010'; // center guard bar
			for ($i = $half_len; $i < $len; ++$i) {
				$seq .= $codes['C'][$code[$i]];// php 7.4
			}
		...
		for ($i = 0; $i < $clen; ++$i) {
			$w += 1;
			if (($i == ($clen - 1)) OR (($i < ($clen - 1)) AND ($seq[$i] != $seq[($i+1)]))) {// php 7.4
				if ($seq[$i] == '1') {// php 7.4
					$t = true; // bar
				} else {
					$t = false; // space
	...
	protected function barcode_rms4cc($code, $kix=false) {
		...
		if ($notkix) {
			...
			$col = 0;
			for ($i = 0; $i < $len; ++$i) {
				$row += $checktable[$code[$i]][0];// php 7.4
				$col += $checktable[$code[$i]][1];// php 7.4
			}
			$row %= 6;
			$col %= 6;
		...
		for ($i = 0; $i < $len; ++$i) {
			for ($j = 0; $j < 4; ++$j) {
				switch ($barmode[$code[$i]][$j]) {// php 7.4
					case 1: {
						$p = 0;
						$h = 2;
						break;
	...
	protected function barcode_codabar($code) {
		...
		$len = strlen($code);
		for ($i = 0; $i < $len; ++$i) {
			if (!isset($chr[$code[$i]])) {// php 7.4
				return false;
			}
			$seq = $chr[$code[$i]];// php 7.4
			for ($j = 0; $j < 8; ++$j) {
				if (($j % 2) == 0) {
					$t = true; // bar
				} else {
