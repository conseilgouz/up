...
(function($) {
    var Toc = function(element, options) {
        ...
        // version LM
        var zheaders = $(this.options.content).find(this.options.selector);
        var that = this;
        zheaders.each(function (index, node) {
            if (!$(node).is(that.options.exclude)) {
                $(node)
...
//
    ...
    Toc.DEFAULT_OPTIONS = {
        content: 'body', // add LM
        exclude: '.notoc', // add LM
        selector: 'h2, h3, h4, h5, h6',
        maxlen: 99,
        elementClass: 'uptoc', // LM
        rootUlClass: 'toc-ul-root',
        ulClass: 'toc-ul',
                levelPrefixClass: 'toc-level-',
        heading: null,
    ...
    Toc.prototype = {
        ...
        render: function() {
            ...
            for (var i = 0; i < numHeadings; i++) {
                ...
                var title = '';
                if (text.length > this.options.maxlen) {
                    title = text;
                    text = text.slice(0, this.options.maxlen) + '&#x2026';
                }
                var id        = this.generateHeadingId(headings[i]),
                    numbering = String($(headings[i]).data('numbering')).split('.'),
//                        $a = $('<a/>').html($(headings[i]).text().slice(0, this.options.maxlen))
                ...
                if (numbering.length == 1) {
                    var $li = $('<li/>').addClass(this.options.levelPrefixClass + 1).wrapInner($a).appendTo($toc); // LM
                } else {
...
//                        $li  = $('<li/>').wrapInner($a).appendTo($ul);
                            $li = $('<li/>').addClass(this.options.levelPrefixClass + (numbering.length + 1)).wrapInner($a).appendTo($ul); // LM
